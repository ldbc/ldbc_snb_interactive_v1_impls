...
BEGIN
    UPDATE Person_knows_Person
    SET weight = weight + 1
    WHERE Person1Id IN (SELECT CreatorPersonId FROM INSERTED)
    AND Person2Id IN (SELECT CreatorPersonId FROM Message WHERE MessageId IN (SELECT ParentMessageId FROM INSERTED))

    INSERT INTO Message_replyOf_Message($from_id, $to_id)
    VALUES
    (
        (SELECT $NODE_ID FROM Message WHERE MessageId IN (SELECT MessageId FROM INSERTED)),
        (SELECT $NODE_ID FROM Message WHERE MessageId IN (SELECT ParentMessageId FROM INSERTED))
    )
END;
