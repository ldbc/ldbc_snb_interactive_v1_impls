...
//        ti.shutdown(); //Use this code to build a graph and load data to it on the server
        ...
        URL workloadPropsPath = this.getClass().getClassLoader().getResource("workloads/ldbc_driver_debug.properties"); //"workloads/ldbc_snb_interactive.properties");
        URL driverPropsPath = this.getClass().getClassLoader().getResource("workloads/ldbc_snb_Q2.properties");
        URL validationParamsPath = this.getClass().getClassLoader().getResource("workloads/validation_params.csv");
        String updateQPath = "N:/SharedData/ALP2/LDBC/1KP/updateStream.properties";

        if (workloadPropsPath == null)
            logger.error("invalid path for workload properties");
        else if (driverPropsPath == null)
