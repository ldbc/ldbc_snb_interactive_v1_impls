...
if ${USE_DATAGEN_DOCKER:-false}; then
    ...
    DATAGEN_COMMAND="docker run --volume `pwd`/out:/${LDBC_SNB_DATAGEN_DIR}/out-sf${SF} ldbc/datagen-standalone:latest --parallelism $(nproc) --"
else
    echo "Using non-containerized Datagen"
    cd ${LDBC_SNB_DATAGEN_DIR}
    export LDBC_SNB_DATAGEN_JAR=$(sbt -batch -error 'print assembly / assemblyOutputPath')
    DATAGEN_COMMAND="tools/run.py --cores $(nproc) --memory ${LDBC_SNB_DATAGEN_MAX_MEM} -- --output-dir ${LDBC_SNB_DATAGEN_DIR}/out-sf${SF}"
fi
