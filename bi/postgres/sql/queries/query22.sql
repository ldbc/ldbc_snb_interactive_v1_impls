...
and contacted.p_placeid = target.ctry_city and target.ctry_name = '--1--'
and contact.p_placeid = source.ctry_city and source.ctry_name = '--2--'
group by contact.p_personid, contact.p_firstname, contact.p_lastname,contact.p_placeid
order by s desc
)
select p_personid, total.score  
