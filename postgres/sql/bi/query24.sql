select extract(year from ps_creationdate) as yy, extract(month from ps_creationdate) as mm, continent, count (*) as n_posts, 
...
where pst_tagid in
     ...
     where tc_name = '--1--' and  ttc_tagclassid = tc_tagclassid)
group by extract(year from ps_creationdate), extract(month from ps_creationdate), continent 
order by extract(year from ps_creationdate), extract(month from ps_creationdate), continent;
