...
WITH replies_w_tags AS (
    SELECT p.p_personid AS creatorid
         ...
         , r.ps_replyof AS replyof
         , r.ps_content AS content
         , CASE
             WHEN count(pt.pst_tagid)=0 THEN ARRAY[]::bigint[] -- no tags for the message, so we return an empty array
             ELSE array_agg(pt.pst_tagid)
           END AS replyTags
      FROM place co -- country
         , place ci -- city
         , person p
         , post r -- reply
           LEFT JOIN post_tag pt ON (r.ps_postid = pt.pst_postid)
     WHERE 1=1
        -- join
     ...
     GROUP BY p.p_personid, r.ps_postid, r.ps_replyof, r.ps_content
)
    ...
    SELECT r.replyid
      ...
      FROM replies_blacklist_excluded_2 r
         , post b -- base message of the reply
           LEFT JOIN post_tag pt ON (b.ps_postid = pt.pst_postid)
     WHERE 1=1
        -- join
...
SELECT r.creatorid AS "person.id"
  ...
  FROM replies_not_sharing_tags_w_base_message r
       LEFT JOIN likes l ON (r.replyid = l.l_postid)
     , post_tag pt
     , tag t
 WHERE 1=1
    -- join
   AND r.replyid = pt.pst_postid
