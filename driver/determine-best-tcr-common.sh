...
for i in $(seq 1 ${TCR_STEPS}); do
    echo "[determine-best-tcr.sh] Step ${i}" | tee tcr.log
    echo "[determine-best-tcr.sh] - Current TCR value: ${TCR_CURRENT}" | tee tcr.log
    ...
    # Conduct benchmark run
    driver/benchmark.sh | tee /tmp/ldbc_snb_driver_output

    # grep for SCHEDULE AUDIT PASSED, looking for 2 lines
    grep 'SCHEDULE AUDIT' /tmp/ldbc_snb_driver_output > /tmp/ldbc_snb_schedule_audit
    if [ "$(wc -l < /tmp/ldbc_snb_schedule_audit)" != "2" ]; then
        echo "[determine-best-tcr.sh] Error: Driver output should contain two lines with SCHEDULE AUDIT" | tee tcr.log
        break
    fi
    ...
    if $(tail -n +2 /tmp/ldbc_snb_schedule_audit | grep 'PASSED SCHEDULE AUDIT'); then
        echo "[determine-best-tcr.sh] Passed schedule audit for TCR value: ${TCR_CURRENT}" | tee tcr.log

        if [ ${TCR_CURRENT} = ${TCR_MIN} ]; then
            echo "[determine-best-tcr.sh] Run passed with the minimum TCR value, no need for further runs" | tee tcr.log
            break
        fi

        TCR_CURRENT=$(echo "scale=4 ; ${TCR_MIN} + (${TCR_CURRENT} - ${TCR_MIN}) / 2" | bc | awk '{printf "%.4f\n", $0}')
    else
        echo "[determine-best-tcr.sh] Failed schedule audit for TCR value: ${TCR_CURRENT}" | tee tcr.log
